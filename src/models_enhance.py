...
class GraphSageEnhanced(nn.Module):
	"""docstring for GraphSage"""
	def __init__(self, num_layers, input_size, out_size, raw_features, adj_lists, device, gcn=False, agg_func='MEAN'):
		super(GraphSageEnhanced, self).__init__()
	...
	def forward(self, nodes_batch, minEnhanced = [], minNodeSelfFeature = [], needIndx = False):
		"""
		Generates embeddings for a batch of nodes.
		nodes_batch	-- batch of nodes to learn the embeddings
		minEnhanced -- enhance node neighour feature
		targetLabel -- enhance label
		"""
		...
		record_embs = []
		for index in range(1, self.num_layers+1): # todo 就一层 搞起
		...
		#print('aggregate_feats' + str(aggregate_feats.shape))
		for minE in minEnhanced:
			aggregate_feats = torch.mean(minE, dim=1)
			# todo 这个要从最外层网 最里层 搞
			cur_hidden_embs_enhanced = self.sage_layer1(self_feats=minNodeSelfFeature,
													aggregate_feats=aggregate_feats)
			# GraphSage 使用 pre_hidden_embs
		if len(minEnhanced) > 0:
			pre_hidden_embs_enhanced = torch.cat([pre_hidden_embs, cur_hidden_embs_enhanced], dim=0)
		else:
			pre_hidden_embs_enhanced = pre_hidden_embs

		# 所以只要 pre_hidden_embs 和 pre_feature
		return pre_hidden_embs_enhanced, pre_feature, samp_neighs, mask
...
class GraphSageEnhancedMulti(nn.Module):
	"""docstring for GraphSage"""

	def __init__(self, num_layers, input_size, out_size, raw_features, adj_lists, device, dim, padding, gcn=False, agg_func='MEAN'):
		super(GraphSageEnhancedMulti, self).__init__()
	...
	def forward(self, nodes_batch, minEnhanced=[], minRecord=[], needIndx = False):
		"""
		Generates embeddings for a batch of nodes.
		nodes_batch	-- batch of nodes to learn the embeddings
		minEnhanced -- enhance node neighour feature
		...
		for index in range(1, self.num_layers + 1):  # todo 就一层 搞起
			...
			if index > 1:
				...
				for nbs in nb:
					count_idx += 1
					if nbs in minRecord:
						do_operation.append( nbs ) # 要知道是哪一个 节点
						do_operation_idx[nbs] = minRecord.index(nbs)
						record_embs.append( cur_hidden_embs[nbs] )
				count += 1


			# todo 多层 是集合最外层两层的特征 所以都需要
			# 如果层数是1 aggregate_feats 是特征 cur_hidden_embs 是聚合特征
			# 好吧 如果层数为1 只有一个 samp_neighs 是邻居信息 可以参考 agg里面怎么操作的
			sage_layer = getattr(self, 'sage_layer' + str(index))

			# self.dc.logger.info('sage_layer.')
			tmp = pre_hidden_embs[nb]
			cur_hidden_embs = sage_layer(self_feats=pre_hidden_embs[nb],
										 aggregate_feats=aggregate_feats)
			pre_hidden_embs = cur_hidden_embs
		...
		# todo 1 替换某几个节点
		# todo 2 直接用全部的
		cur_hidden_embs_enhancedS = []
		enhance_idx = []

		if len(minEnhanced) > 0:

			for minEs in range(0, (minEnhanced[0].size()[0])):
				if minRecord[minEs] in do_operation: # 若需要进行增强
					#print (minRecord[minEs])
					tmp1 = do_operation.index(minRecord[minEs])
					enhance_idx.append(do_operation_idx[minRecord[minEs]])
					if minRecord[minEs] == 969:
						z = 1
					minE = minEnhanced[0][minEs]
					# minNodeSelfFeature 这个使用之前的 cur_hidden
					minE_1 = minE[:,:self.dim]
					minE_2tmp = minE[:,self.dim:]# 第二层
					minE_2 = minE_2tmp.view(self.padding, self.padding, self.dim)  # 当某一维为-1的时候，会自动计算它的大小

					aggregate_feats_2 = torch.mean(minE_2, dim=1)
					# todo 这个要从最外层网 最里层 搞
					cur_hidden_embs_enhanced_2 = self.sage_layer1(self_feats=minE_1,
																aggregate_feats=aggregate_feats_2)
					aggregate_feats_1 = torch.mean(cur_hidden_embs_enhanced_2, dim=0)
					minE = record_embs[do_operation.index(minRecord[minEs])]

					minE = torch.unsqueeze(minE, 0)
					aggregate_feats_1 = torch.unsqueeze(aggregate_feats_1, 0)
					#print (minE[0].size())
					#print (aggregate_feats_1[0].size())
					cur_hidden_embs_enhanced = self.sage_layer2(self_feats=minE,
																  aggregate_feats=aggregate_feats_1)
					cur_hidden_embs_enhanced = cur_hidden_embs_enhanced.squeeze()
					cur_hidden_embs_enhancedS.append(cur_hidden_embs_enhanced)

		# GraphSage 使用 pre_hidden_embs
		if len(cur_hidden_embs_enhancedS) > 0:
			cur_hidden_embs_enhancedSTensor2 = torch.tensor([item.cpu().detach().numpy() for item in cur_hidden_embs_enhancedS])
			cur_hidden_embs_enhancedSTensor = cur_hidden_embs_enhancedSTensor2.cuda()
			pre_hidden_embs_enhanced = torch.cat([pre_hidden_embs, cur_hidden_embs_enhancedSTensor], dim=0)
		else:
			pre_hidden_embs_enhanced = pre_hidden_embs

		# 所以只要 pre_hidden_embs 和 pre_feature
		if needIndx:
			return pre_hidden_embs_enhanced, pre_feature, samp_neighs, mask, enhance_idx
		else:
			return pre_hidden_embs_enhanced, pre_feature, samp_neighs, mask
...
class GraphSageEnhancedPPITwo(nn.Module):
	"""docstring for GraphSage"""

	def __init__(self, num_layers, input_size, out_size, raw_features, adj_lists, device, dim, padding, gcn=False, agg_func='MEAN'):
		super(GraphSageEnhancedPPITwo, self).__init__()
	...
	def forward(self, nodes_batch, minEnhanced=[], minRecord=[], needIndx = True):
		"""
		Generates embeddings for a batch of nodes.
		nodes_batch	-- batch of nodes to learn the embeddings
		minEnhanced -- enhance node neighour feature
